 ...
 * @param[in] p_ble_evt  Event received from the BLE stack.
 */
static void on_write(ble_lbs_t * p_lbs, uint8_t button_state)
{
    if ((button_state == 1)&&			
		    (p_lbs->led_write_handler != NULL))
    {
        p_lbs->led_write_handler(p_lbs,1);
    }
		else
		p_lbs->led_write_handler(p_lbs,0);
}
 ...
 * @retval NRF_SUCCESS on success, else an error value from the SoftDevice
 */
static uint32_t mode_char_add(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ble_gatts_attr_t    attr_char_value;
    ...
    ble_uuid.type = p_lbs->uuid_type;
    ble_uuid.uuid = LBS_UUID_MODE_CHAR;
...
static uint32_t button_char_add(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ...
    ble_uuid.type = p_lbs->uuid_type;
    ble_uuid.uuid = LBS_UUID_SENSOR_CHAR;
...
uint32_t ble_lbs_init(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ...
    VERIFY_SUCCESS(err_code);
	  ...
	  err_code = mode_char_add(p_lbs, p_lbs_init);
    VERIFY_SUCCESS(err_code);
...
uint32_t ble_lbs_on_mode_change(ble_lbs_t * p_lbs, uint8_t button_state)
{
    ...
    memset(&params, 0, sizeof(params));
    params.type = BLE_GATT_HVX_NOTIFICATION;
    params.handle = p_lbs->mode_char_handles.value_handle;
